...
import { DivisionType, Division } from '../model/Division';
...
export class DivisionAgeLimit1507982943458 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await query
      ...
      .where({name: 'defaultValues'})
      .execute();
      await query.update(Division).set({ min: 8, max: 11}).where({name: 'Aspirant'}).execute();
      await query.update(Division).set({ min: 11, max: 13}).where({name: 'Rekrutt'}).execute();
      await query.update(Division).set({ min: 13, max: 17}).where({name: 'Junior'}).execute();
      await query.update(Division).set({ min: 16, max: 99}).where({name: 'Senior'}).execute();
      await query.delete().from(Configuration).where({name: 'ageLimits'}).execute();
  }
